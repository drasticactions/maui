...
namespace Microsoft.Maui
{
	internal class AdornerService : IAdornerService
	{
		...
		{
			if (window.Content is not ILayout layout)
				return null;
		...
		IView? FindIViewAt(IView? view, int x, int y)
		{
			...
			if (view is IVisualTreeElement layout)
			{
				var children = layout.GetVisualChildren();
				for (int i = 0; i < children.Count; i++)
				{
					if (children[i] is IView childView)
					{
						IView? foundView = FindIViewAt(childView, x, y);
